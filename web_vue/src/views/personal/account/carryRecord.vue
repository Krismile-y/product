<template>
  <div class="carryRecord">
    <Header title="Bản ghi số dư có thể rút" color="transparent" fontColor="#fff" fontSize="20px" iconSize="30px" :fixedFlag="true"></Header>
    <div class="content">
      <div class="bg">
        <!--        <img src="../../../assets/images/recordBg.png" alt="">-->
        <div>
          <i>{{ totalPrice }}</i>
          <span>Số dư rút tiền của tôi</span>
        </div>
      </div>
      <ul>
...
<!--            <span style="font-size: 12px;">修改前：{{ item.upd_a }}</span>-->
<!--            <span style="font-size: 12px;">修改后：{{ item.upd_b }}</span>-->
<!--          </div>-->
<!--        </li>-->
<!--      </ul>-->
  ...
  methods: {
    async submitPage() {
      if (parseInt(this.dialogPage) > this.carryList.last_page) {
        ...
        let res = await axios.post('api/member/get_jf', {
          user_id: userId,
