<template>
  <div class="yueRecord">
    <Header title="Hồ sơ cân bằng" color="transparent" fontColor="#fff" fontSize="20px" iconSize="30px" :fixedFlag="true"></Header>
    <div class="content">
      <div class="bg">
<!--        <img src="../../../assets/images/recordBg.png" alt="">-->
      ...
      <ul>
        <li v-for="(item) in yueList.data" :key="item.id">
          <div class="left">
            <!--  其他消费图片  -->
          ...
          <div class="right">
            <div class="top">
              <!-- <span>{{ item.mone.split('(')[0] }} {{ item.name }}</span> -->
              <span>Mua hàng tiêu dùng {{ item.name }}</span>
...
export default {
  ...
  async created() {
    ...
    if (this.page === 1 && this.yueList.length === 0) {
  ...
  methods: {
    // 弹框确认事件
    async submitPage() {
      if (parseInt(this.dialogPage) > this.yueList.last_page) {
        this.$toastBounce('Vui lòng nhập đúng số trang!')
        return
      } else {
        const userId = JSON.parse(sessionStorage.getItem('userInfo')).id;
        let res = await axios.post('api/member/my_yy', {
          user_id: userId,
          page: this.dialogPage,
          limit: this.limit
        })
        if (res.data.code === 200) {
          this.yueList = res.data.data
    ...
    // 下一页事件
    async nextClick() {
      console.log(this.yueList);
      if (this.yueList.current_page === this.yueList.last_page) {
