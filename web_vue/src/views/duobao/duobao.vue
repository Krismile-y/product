<template>
  <div class="duobao">
    <Header title="Bản ghi vàng Duobao" color="transparent" fontColor="#fff" fontSize="20px" iconSize="30px" :fixedFlag="true"></Header>
    <div class="content">
      <div class="bg">
        <!--        <img src="../../../assets/images/recordBg.png" alt="">-->
        <div>
          <i>{{ totalPrice }}</i>
          <span>Vàng kho báu của tôi</span>
        </div>
      </div>
      <ul>
        <li v-for="(item) in duobaoList.data" :key="item.id">
          <div class="left">
            <!--  邀请获得图片  -->
...
<!--              <img src="../../assets/images/zengjia.png" alt="">-->
            <!--            &lt;!&ndash;  减值图片  &ndash;&gt;-->
            <img src="../../assets/images/jianshao.png" alt="">
          </div>
          <div class="right">
            <div class="top">
              <span>{{ item.mone.split('(')[0] }}</span>
...
export default {
  ...
  async created() {
    ...
    if (this.page === 1 && this.duobaoList.data.length === 0) {
  ...
  methods: {
    async submitPage() {
      if (parseInt(this.dialogPage) > this.duobaoList.last_page) {
        this.$toastBounce('Vui lòng nhập đúng số trang!')
        return
      } else {
        const userId = JSON.parse(sessionStorage.getItem('userInfo')).id;
        let res = await axios.post('api/member/get_jf2', {
          user_id: userId,
    ...
    // 下一页事件
    async nextClick() {
      if (this.duobaoList.current_page === this.duobaoList.last_page) {
