<template>
  <div class="chongzhiRecord">
    <Header title="hồ sơ nạp tiền" color="transparent" fontColor="#fff" fontSize="20px" iconSize="30px" :fixedFlag="true"></Header>
    <div class="content">
      <div class="bg">
        <div>
          <i>{{ totalPrice }}</i>
      ...
      <ul>
        <li v-for="(item) in chongzhiList.data" :key="item.id">
          <div class="left">
            <!--  其他消费图片  -->
            <img v-if="item.type === 4" src="../../assets/images/qita.png" alt="">
            <!--  充值图图片  -->
          ...
          <div class="right">
            <div class="top">
              <span>{{ item.mone.split('(')[0] }} {{ item.name }}<span :style="{ color: getColor(item) }">[{{ getStatus(item) }}]</span></span>
              <!--              <span v-if="item.goods_name">{{ item.goods_name ? item.goods_name : '' }}</span>-->
              <span :style="{ color: parseFloat(item.auto_price) > 0 ? '#4FBF03' : '#000' }">{{ item.number !== '0.00' ? item.number : item.auto_price | priceFuhao }}</span>
            </div>
            <div class="bottom">
...
export default {
  ...
  async created() {
    ...
    if (this.page === 1 && this.chongzhiList.data.length === 0) {
  ...
  methods: {
    ...
    // 弹框确认事件
    async submitPage() {
      if (parseInt(this.dialogPage) > this.chongzhiList.last_page) {
        this.$toastBounce('Vui lòng nhập đúng số trang!')
        return
      } else {
        const userId = JSON.parse(sessionStorage.getItem('userInfo')).id;
        let res = await axios.post('api/member/get_cz_log', {
          user_id: userId,
        ...
        if (res.data.code === 200) {
          ...
          console.log('chongzhiList',chongzhiList);
    ...
    // 下一页事件
    async nextClick() {
      console.log(this.chongzhiList);
      if (this.chongzhiList.current_page === this.chongzhiList.last_page) {
